...
const periods = [
  { id: "period-1", startTime: "09:00", endTime: "09:50" }, // 50 minutes
  { id: "period-2", startTime: "10:00", endTime: "10:40" }, // 40 minutes
];

describe("computeScheduledMinutesForClass", () => {
  it("multiplies a period's duration by how many times its cycle day occurs", () => {
    const scheduleSlots = [{ classId: "class-1", periodId: "period-1", cycleDay: 1 }];

    // 50 minutes x 5 occurrences of cycle day 1.
    expect(computeScheduledMinutesForClass(schoolYear, "class-1", periods, scheduleSlots)).toBe(
      250,
    );
  });
  ...
  it("sums across multiple slots for the same class", () => {
    const scheduleSlots = [
      { classId: "class-1", periodId: "period-1", cycleDay: 1 },
      { classId: "class-1", periodId: "period-2", cycleDay: 2 },
    ];
  ...
  it("ignores slots belonging to other classes", () => {
    const scheduleSlots = [
      { classId: "class-1", periodId: "period-1", cycleDay: 1 },
      { classId: "class-2", periodId: "period-2", cycleDay: 2 },
    ];
  ...
  it("returns 0 for a class with no scheduled slots", () => {
    expect(computeScheduledMinutesForClass(schoolYear, "class-1", periods, [])).toBe(0);
  });

  it("skips a slot referencing a period that no longer exists", () => {
    const scheduleSlots = [{ classId: "class-1", periodId: "deleted-period", cycleDay: 1 }];
...
describe("computeInstructionalTimeSummary", () => {
  it("flags a class as meeting its target when scheduled minutes are enough", () => {
    ...
    const [summary] = computeInstructionalTimeSummary(schoolYear, classes, periods, scheduleSlots);
  ...
  it("treats a class with no target as always meeting it (informational only)", () => {
    ...
    const [summary] = computeInstructionalTimeSummary(schoolYear, classes, periods, []);
